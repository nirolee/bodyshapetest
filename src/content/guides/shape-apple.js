export default { path: '/shapes/apple/', en: {
 ...
 "zh": {
  "meta": {
   "title": "蘋果型身材（Oval 與 Diamond）：多數計算機沒有的規則",
   "description": "蘋果型是本網站對 FFIT 結果 Oval 與 Diamond 的歸類，此時腰圍大於臀圍。本文說明規則、兩者差異、為何 2007 年公式組以不同方式處理這種身材。"
  },
  "layout": {
   "eyebrow": "身材 · 蘋果型",
   "h1": "蘋果型身材",
   "lead": "在本網站中，蘋果型是兩種 FFIT 結果 Oval 與 Diamond 的常用名稱歸類，兩者皆為腰圍大於臀圍。兩者均屬於 2004 年 FFIT 身材組合，但未納入部分計算機採用的 2007 年公式組；2020 年修訂版重新納入了它們。由於規則會依序檢查，腰圍大於臀圍時仍可能先符合較早的規則。以下說明這兩項規則、兩者的差異，以及它與 Rectangle 結果的不同。",
  ...
  "blocks": [
   ...
   {
    "t": "p",
    "s": "它是三個圍度之間的關係。它不測量或代表體重、健康狀況，或寬度在前後方向的位置；也不是其他身材用來比較的類別。穿搭頁面列出例如中段採用較長線條或選擇較高腰線等選項；這些是偏好，不是指示。"
   },
   {
    "t": "ul",
    "items": [
